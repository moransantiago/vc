...
import ContentLoader, { rect } from 'reac-content-loader'
...
export const ServersColumn = ({ serverId, chatId }) => {
	...
	useEffect(() => {
		return () => {
			if (serversSocket && connectedChannel) {
	...
	}, [obtainSocket, connectedChannel, userId])
	...
	return (
		...
		>
			{({ loading, error, data }) => {
				...
				const handleDisconnection = channelId => {
					RTCsocket.emit('left', { id: data.getMe._id })
					serversSocket.emit('leave_channel', { userId: data.getMe._id, channel: channelId })
										...
										>
											...
											>
												{connectedChannel === channel._id &&
													<DisconnectButton onClick={() => handleDisconnection(channel._id)}>
														<MdCallEnd size='16' color='inherit' />
													</DisconnectButton>}
											</Card>
										</Button>
										<div>
											{channel.connectedUsers && channel.connectedUsers.map((user, index) => <p key={index}>{user}</p>)}
										</div>
									</div>
								))}
							</div>
