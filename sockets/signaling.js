	...
	bindSignalingEvents: io => {
		...
		io.on('connect', socket => {
			...
			socket.on('join', room => {
				...
				socket.join(room, () => {
					const amountOfSocketsInRoom = Object.keys(io.adapter.rooms[room].sockets).length // => This is the amount of sockets joined to this room
					...
					socket.broadcast.to(room).emit('joiner') // => Broadcast to everyone in the room that a new user has joined
				})
			})
		})
	}
